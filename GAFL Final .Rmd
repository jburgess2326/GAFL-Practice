...
```
```{r}
# vectors for the variables that are in the cleaned data
...
```{r} 
# putting results into latex 

library(stargazer)

# Define groups manually based on your Excel
land_clearing_dvs <- c("fclear_yn", "fclear_punishyn", "fclear_punish", "fclear_rulebreak", "fclear_rulefollow",
                       "fclear_who", "fclear_whomost", "fhuntfire_yn", "fhuntfire_punishyn", "fhuntfire_punish",
                       "fhuntfire_rulebreak", "fhuntfire_rulefollow", "fhuntfire_who", "fhuntfire_whomost",
                       ...
                       "ffire_rulebreak", "ffire_rulefollow", "ffire_who", "ffire_whomost", "hfrstserv_ben_2",
                       "hfrstserv_ben_3", "hfrstserv_ben_4", "parkatt_waste", "hwhoperm", "hallocateoften",
                       "hnewclear_a", "hcleardriver1", "hcleardriver2", "hrank_farmland", "wood_unauth",
                       "fpolicy_action")

forest_governance_dvs <- c("hfsupport", "hfsupport_who", "hfsupport_type", "hfrstserv_ben_1", "hfrstserv_ben_2",
                           "hfrstserv_ben_3", "hfrstserv_ben_4", "hfrstserv_ben_5", "hfrstserv_ben_6",
                           "htourism_pay", "htourism_whoreceive", "htourism_amt", "hcarbon_pay",
                           "hcarbon_whoreceive", "hcarbon_amt", "htimber_co", "htimber_pay",
                           "htimber_whoreceive", "htimber_amt", "hmine_co", "hmine_pay", "hmine_whoreceive",
                           "hmine_amt", "forest_noaccess", "noaccwhy", "noaccimpact", "forstrict_who",
                           "forguard", "ccrb_yn", "ccrb_name", "vag_yn", "vag_name", "vag_meet", "vag_whoattnd",
                           "vag_women", "vag_womrestrict", "vag_youth", "govern_foruse", "gov_change",
                           "gov_changewho", "fparticipateyn", "ftalk_who", "hunt_unauth", "fpolicy_effective",
                           "wood_unauth", "fpolicy_action")

livelihoods_dvs <- c("hfrstserv_ben_6", "watt_tour", "watt_lodge", "crbatt_employ", "hunt_prestige",
                     "conserve_neg", "parkatt_help", "parkatt_employ", "parkatt_meat", "parkatt_adverse",
                     "parkatt_outside", "hrank_job", "fhhinc_timber", "fhhinc_nontimber",
                     "fincomeprod_1", "fincomeprod_2", "hproblem_job")

health_outcomes_dvs <- c("parkatt_harrass", "hnear_primschool", "hnear_secschool", "hnear_road", "hnear_mkt",
                         "hrank_health", "hrank_edu", "watsource", "watsource_prim", "watsource_hhnum",
                         "watsource_safe", "watsource_whysafe", "watsource_whynosafe", "borehole_func",
                         "borehole_nonfunc", "borehole_mgmt", "borehole_chall_yn", "borehole_chall",
                         "borehole_seaschng", "borehole_yrchng", "health_dist", "health_trans", "health_qual",
                         "health_out", "health_outfreq", "health_outtype", "health_chw", "health_chwtype",
                         "health_chwfreq", "health_chwsvc", "comsvc", "comsvc_watsan_yr", "comsvc_watsan_imp",
                         "comsvc_energy_yr", "comsvc_energy_imp", "comsvc_edu_yr", "comsvc_edu_imp",
                         "comsvc_health_yr", "comsvc_health_imp", "comsvc_mgmt")

# Group into a list
groups <- list(
  LAND_CLEARING = land_clearing_dvs,
  FOREST_GOVERNANCE = forest_governance_dvs,
  LIVELIHOODS = livelihoods_dvs,
  HEALTH_OUTCOMES = health_outcomes_dvs
)

# Now generate LaTeX tables
for (group_name in names(groups)) {
  dv_list <- groups[[group_name]]
  
  # Filter models for this group
  group_models <- models_sig[names(models_sig) %in% dv_list]
  
  if (length(group_models) > 0) {
    # Create the table
    cat("\\begin{table}[!htbp]\n\\centering\n\\small\n")
    cat(stargazer(group_models,
                  type = "latex",
                  title = paste("Significant Models for", gsub("_", " ", group_name)),
                  model.names = FALSE,
                  column.labels = names(group_models)))
    cat("\\end{table}\n\n")
  }
}
